...
def write_to_file(ip_address, enum_type, data):
    ...
    file_path_linux = '/opt/ctf/results/%s/mapping-linux.md' % (ip_address)
    file_path_windows = '/opt/ctf/results/%s/mapping-windows.md' % (ip_address)
...
def dirb(ip_address, port, url_start):
    ...
    DIRBSCAN = "dirb %s://%s:%s -o /opt/ctf/results/%s/dirb-%s.txt -r" % (url_start, ip_address, port, ip_address, ip_address)
...
def nikto(ip_address, port, url_start):
    ...
    NIKTOSCAN = "nikto -h %s://%s -o /opt/ctf/results/%s/nikto-%s-%s.txt" % (url_start, ip_address, ip_address, url_start, ip_address)
...
def httpEnum(ip_address, port):
    ...
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt,http-devframework,http-enum,http-frontpage-login,http-git,http-iis-webdav-vuln,http-php-version,http-robots.txt,http-shellshock,http-vuln-cve2015-1635 -oN /opt/ctf/results/%s/%s_http.nmap %s" % (port, ip_address, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    SSLSCAN = "sslscan %s:%s >> /opt/ctf/results/%s/ssl_scan_%s" % (ip_address, port, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oN /opt/ctf/results/%s/mssql_%s.nmap %s" % (port, ip_address, ip_address)
...
def smtpEnum(ip_address, port):
    ...
    SMTPSCAN = "nmap -sV -Pn -p %s --script=smtp-commands,smtp-enum-users,smtp-vuln-cve2010-4344,smtp-vuln-cve2011-1720,smtp-vuln-cve2011-1764 %s -oN /opt/ctf/results/%s/smtp_%s.nmap" % (port, ip_address, ip_address, ip_address)
...
def smbEnum(ip_address, port):
    print "INFO: Detected SMB on " + ip_address + ":" + port
    enum4linux = "enum4linux -a %s > /opt/ctf/results/%s/enum4linux_%s" % (ip_address, ip_address, ip_address)
...
def ftpEnum(ip_address, port):
    ...
    FTPSCAN = "nmap -sV -Pn -vv -p %s --script=ftp-anon,ftp-bounce,ftp-libopie,ftp-proftpd-backdoor,ftp-vsftpd-backdoor,ftp-vuln-cve2010-4221 -oN '/opt/ctf/results/%s/ftp_%s.nmap' %s" % (port, ip_address, ip_address, ip_address)
...
def udpScan(ip_address):
    ...
    UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN '/opt/ctf/results/%s/udp_%s.nmap' %s"  % (ip_address, ip_address, ip_address)
    ...
    UNICORNSCAN = "unicornscan -mU -v -I %s > /opt/ctf/results/%s/unicorn_udp_%s.txt" % (ip_address, ip_address, ip_address)
...
def nmapScan(ip_address):
    ...
    TCPSCAN = "nmap -sV -O %s -oN '/opt/ctf/results/%s/%s.nmap'"  % (ip_address, ip_address, ip_address)
...
if __name__=='__main__':
    ...
    dirs = os.listdir("/opt/ctf/results")
    for scanip in targets:
        ...
        if not scanip in dirs:
            ...
            subprocess.check_output("mkdir /opt/ctf/results/" + scanip, shell=True)
            subprocess.check_output("mkdir /opt/ctf/results/" + scanip + "/exploits", shell=True)
            subprocess.check_output("mkdir /opt/ctf/results/" + scanip + "/privesc", shell=True)
            print bcolors.OKGREEN + "INFO: Folder created here: " + "/opt/ctf/results/" + scanip + bcolors.ENDC
            subprocess.check_output("cp /opt/ctf/templates/windows-template.md /opt/ctf/results/" + scanip + "/mapping-windows.md", shell=True)
            subprocess.check_output("cp /opt/ctf/templates/linux-template.md /opt/ctf/results/" + scanip + "/mapping-linux.md", shell=True)
            print bcolors.OKGREEN + "INFO: Added pentesting templates: " + "/opt/ctf/results/" + scanip + bcolors.ENDC
            subprocess.check_output("sed -i -e 's/INSERTIPADDRESS/" + scanip + "/g' /opt/ctf/results/" + scanip + "/mapping-windows.md", shell=True)
            subprocess.check_output("sed -i -e 's/INSERTIPADDRESS/" + scanip + "/g' /opt/ctf/results/" + scanip + "/mapping-linux.md", shell=True)
